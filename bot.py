"""
Sebastian Thornmere - the shy Thornmere ghost who rebuilt the Tri-Wizard
Tournament. Entry point: wires up the client and loads cogs. He only ever
speaks in response to someone, and he never talks to the other ghosts.
"""
...
log = logging.getLogger("thornmere")
...
bot = commands.Bot(command_prefix="!thornmere-unused-", intents=intents, help_command=None)
...
@bot.event
async def on_ready():
    log.info("Sebastian has arrived. Logged in as %s (id=%s)", bot.user, bot.user.id)
